...
内置分类包括：WEB、H5、全站APP、代理后台、代理H5、管理后台、体育APP、未使用、网站域名、代理域名、站点专属自用-H5、站点专属自用-WEB、站点专属自用-APP、站点专属自用-体育APP、好记-H5 和品牌域名。名称更具体的分类会优先匹配，不会误归入普通 WEB、H5 或 APP。

Excel 中每个分类按“域名、过期时间、日期”三列展示，分类之间保留一列空白，方便区分。日期使用 Excel 日期时间格式。

域名状态为“过期”、到期日期早于生成文件的时间，或剩余天数为负数时，Excel 中对应的域名、过期时间和日期都会显示为红色，并在过期时间后标注“(已过期)”，例如 `61天(已过期)`。到期日期优先用于纠正文本中可能已经过时的剩余天数。
